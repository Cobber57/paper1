...
def create_ixp(filename,ixp_id, ixp_info,facilitys_uk,my_dict):
    ixp_str = str(ixp_id)
    ...
    spacer1 = "        ["
    spacer2 = "],\n"
    print(ixp_info,ixp_id)
    # input ('wait')
    first_facility = str(ixp_info[ixp_id]['fac_set'][0][0])
    first_facility_lat = str(facilitys_uk[first_facility]['latitude'])
    first_facility_lon = str(facilitys_uk[first_facility]['longitude'] )

    
    print('IXP is ', ixp_str, my_dict.keys())
    if ixp_str not in my_dict.keys():
        fac_list = []
        ixp_dict[ixp_id] = {}
        ...
        for fac in ixp_info[ixp_id]['fac_set'][0]:
            fac_str = str(fac)
            ...
            if fac not in fac_list:
                ...
                f = facilitys_uk[fac_str]
                print('facilitys_uk',f)
                ...
                + str(ixp_id)
                + "<br />"
                + ixp_info[ixp_id]['name'][0]
                + "<br />"
                + ixp_info[ixp_id]['ipv4_prefix']
                +"<br />"
                + ixp_info[ixp_id]['ipv6_prefix']
                +"<br />"
                +' Facility '+fac_str
                +"<br />"
                + fac_name
...
'100.124.', '100.125.', '100.126.', '100.127.']
    ...
    filename = 'web_files_results/ixps.html'
    ixp_dict = {}
    create_header_html(filename)
    #try:
    for ixp_id in ixps_uk:
        print('ID is', ixp_id)
        # input('wait')
        if ixp_id != '321' and ixp_id != '53' :
        
            ixp_dict = create_ixp(filename,ixp_id, ixps_uk,facilitys_uk, ixp_dict)
