...
def get_hop_location(ixp_pre_hops,ixp_in_hops,ixp_post_hops,facilities_used,this_target,probe_id,prev_hop,hop,hop_details,ixp_entered_flag):
    ...
    if not local_subnet_flag and not ix_prefix_flag:   
        ...
        #first of all lets check the ip to asn file thats been precreated
    ...
    if not rule_flag:
        ...
        try:
            ...
            results[probe_id]['status'] = False
            print('RULE6')
        # last chances at finding the coords, individual ip addreses
        if results[probe_id]['status'] == False:
            if ipaddress.ip_address(this_hop['ip_from']) in ipaddress.ip_network('185.40.232.255'):
                this_hop['facility'] = '34'
                lon = coords[0]
                lat = coords[1]
                this_hop['facility'].append(this_facility)
                print('RULE 6 after',this_hop['facility'],this_facility)
    ...
    if not already_have_possible_facility:
        ...
        if len(f) > 1:
            ...
            if f[0] == '896' and f[1] == '2384':
                ...
                reverse_measurment = measurements[str(this_measurement)]['results'][this_target]
