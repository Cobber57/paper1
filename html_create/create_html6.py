...
for measurement_id in measurements:
    ...
    for probe_id in measurements[measurement_id]['results']:
        ...
        if int(probe_id) not in uk_probes:
            ...
            for hop in range(hops):
                ...
                while this_rtt == 100:
                    ...
                    if next_h in measurements[measurement_id]['results'][str(probe_id)]['hops']:
                        if measurements[measurement_id]['results'][str(probe_id)]['hops'][next_h]['rtt'] == 100 or measurements[measurement_id]['results'][str(probe_id)]['hops'][next_h]['rtt'] > rtt:
                            count+=1
                            print('this_rtt count',count,this_rtt)
                            continue
                        else:
                            this_rtt = measurements[measurement_id]['results'][str(probe_id)]['hops'][next_h]['rtt']
                            count += 1
                    elif '255' in measurements[measurement_id]['results'][str(probe_id)]['hops']:
                        if measurements[measurement_id]['results'][str(probe_id)]['hops']['255']['rtt'] == 100 or measurements[measurement_id]['results'][str(probe_id)]['hops']['255']['rtt'] > rtt:
                            this_rtt = rtt
                            count += 1 
                        else:
                            this_rtt = measurements[measurement_id]['results'][str(probe_id)]['hops']['255']['rtt']
                ...
                if count == 0:
                    ...
                    if h in measurements[measurement_id]['results'][str(probe_id)]['hops']:
                        ...
                        this_time_taken = this_rtt - measurements[measurement_id]['results'][str(probe_id)]['hops']['255']['rtt']
                    # now add this differnce to the original time but divide it by the number of times had to skip due to 100
                    this_rtt = this_time_taken/count 
                    this_fraction = this_rtt/rtt 
                print('time taken',this_time_taken,'count',count)
                print('total rtt is ',rtt,'this rtt is', this_rtt,'FRACTION', this_fraction)
                print('source target fraction',source_coords, target_coords, this_fraction)
                if source_coords == target_coords or this_fraction == 1:
                ...
                # print('HOP COORDS are', hop_coords)
                # input('wait')
                html.create_hop(probe_id,h,this_hop,this_rtt)
                html.create_lines_var(probe_id,h,lat1,lon1,lat2,lon2,hop_distance,this_rtt,current_ip,this_hop['from'])
                ...
                # Break out in the case of max_hops that equal 255
                if measurement_id == "31906485" and probe_id == '6471':
                    print ('create hop', probe_id,h,this_hop,this_rtt,rtt)
                    print ('create line ', probe_id,h,lat1,lon1,lat2,lon2,hop_distance,this_rtt,current_ip,this_hop['from'] )
                    input('wait')
                if this_fraction == 1:
                    break 
