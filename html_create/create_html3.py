...
class Html_Create:
    ...
    def __init__(self, probe_dict):
        ...
        probe_list = list(probe_dict.keys())  
        print (probe_list)
        print(probe_dict['target_address'])

                        # gets destination address 
        self.target_ip = str(probe_dict['target_address'])                # gets destination address 
        self.target_lat = probe_dict['target_coordinates'][1]
        self.target_lon = probe_dict['target_coordinates'][0]
        ...
        #Create the Probes
        for probe_id in probe_dict['results']:
            ...
            # Create The HOPs
            current_lon = probe_dict['results'][probe_id]['source_coordinates'][1]
            current_lat = probe_dict['results'][probe_id]['source_coordinates'][0]
            current_rtt = 0
            if probe_dict['results'][probe_id]['source_address'] != None:
                current_ip = probe_dict['results'][probe_id]['source_address'] 
            else:
                current_ip = 'unknown'
            hops =  int(probe_dict['results'][probe_id]['max_hops'])
            ...
            for hop in range(hops):
                ...
                # basically 2 ways of working out the lat and lon of the hop. 
                # 1. use nominatum but we know the accuracy isnt always great
                # 2. simply calculate the distance that this hop must be from the last hop
                #    using http://edwilliams.org/avform147.htm#Intermediate
                # The actual location will be diferent but this 'idealised' location will be in the general area
                # Perhaps the 2 differnt methods can be combined. Also once I have a number of 'idealised' 
                # locations with the same ip address i may be able to more accuretly map its location
                # A=sin((1-f)*d)/sin(d)
                # B=sin(f*d)/sin(d)
...
if __name__ == "__main__":
    ...
    for measurement in probes:
        html = Html_Create(probes[measurement])




    '''    
        id = probes[measurement]['target_probe']
        probes[measurement]['target_ip']
        probes[measurement]['target_lat']
        probes[measurement]['target_lon'] 
        probes[measurement]['target_address'] = 'this addresss' 
        probes_dict[id]['probe_asn'] = 0
        probes_dict[id]['probe_ip'] = 
        probes_dict[id]['probe_x'] = probes[measurement]['target_coordinates'][1]
        probes_dict[id]['probe_y'] = probes[measurement]['target_coordinates'][0]
        probes_dict[id]['Hops'] = probes[measurement]['max_hops']
